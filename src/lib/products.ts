/* ============================================================
   BioPlus Labs — Product Catalogue
   Prices are GBP PER VIAL, per the client price list.
   Every vial ships with a batch-matched Certificate of Analysis.
...
export function variantBySku(sku: string): { product: Product; variant: Variant } | undefined {
...
export const stacks = PRODUCTS.filter((p) => p.slug === "glow" || p.slug === "klow");

/** Single-compound best-sellers and new arrivals (excludes blends). */
export const bestSellingPeptides = PRODUCTS.filter(
